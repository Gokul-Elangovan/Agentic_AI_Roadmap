...
const phases = [
  {
    ...
    tracks: [
      ...
      { name: "PostgreSQL & Redis Stack", items: [
        "PostgreSQL setup: install, create DB, roles, pg_hba config",
        "async SQLAlchemy 2.0: AsyncEngine, AsyncSession, async_sessionmaker",
        "SQLAlchemy models: declarative Base, Column types, relationships",
        "Alembic migrations: env.py setup, autogenerate, upgrade/downgrade",
        "Schema design for AI apps: users, conversations, messages, agent_runs tables",
        "JSONB columns: store flexible agent metadata, tool results, config blobs",
        "Connection pooling: pool_size, max_overflow, pool_timeout tuning",
        "Repository pattern: abstract DB queries behind a clean interface",
        "pgvector extension: CREATE EXTENSION, vector column type, operator classes",
        "pgvector indexing: IVFFlat (fast build) vs HNSW (fast query) — when to use each",
        "pgvector queries: <-> cosine, <#> inner product, <+> L2 operators in SQL",
        "Redis data structures: Strings, Hashes, Lists, Sets, Sorted Sets — use cases for each",
        "Redis as cache: SET/GET with TTL, cache-aside pattern, cache invalidation strategies",
        "Redis as session store: store JWT refresh tokens, user sessions with expiry",
        "Redis pub/sub: publish agent events, subscribe from workers — async messaging",
        "Redis as rate limiter: sliding window counter with INCR + EXPIRE pattern",
        "redis-py async client: aioredis, connection pools, pipeline batching",
      ]},
    ...
    hands_on: [
      ...
      { title: "AI App Data Layer Scaffold", desc: "Build the reusable data foundation for every future project: async SQLAlchemy models (users, conversations, messages, agent_runs), Alembic migrations, pgvector extension + vector column + HNSW index, Redis async client with pool, cache-aside helper, sliding-window rate limiter utility. This scaffold gets reused in every project from Phase 3 onward.", tags: ["PostgreSQL", "SQLAlchemy", "Alembic", "pgvector", "Redis", "aioredis"] },
    ],
  },
  {
  ...
  },
  {
    ...
    tracks: [
      ...
      { name: "Embeddings & Vector DBs", items: ["Embedding models: OpenAI text-embedding-3-small/large, local BGE, sentence-transformers","Embedding dimensions: 768 vs 1536 — size, speed, and quality tradeoffs","pgvector (production): PGVector LangChain store, async upsert, metadata filtering in SQL","pgvector performance: lists param tuning for IVFFlat, ef_construction for HNSW","pgvector multi-tenant: filter by tenant_id alongside vector search in one query","Chroma: ephemeral (dev) vs persistent (staging), collections, metadata filters","FAISS: IndexFlatL2 and IndexIVFFlat — build locally, serialize to disk, reload","Pinecone: when managed cloud vector DB makes sense over self-hosted pgvector","Indexing pipeline: chunk → embed → batch upsert with metadata (async, not blocking)","Similarity metrics: cosine vs dot product vs L2 — which to use and why","Vector DB as agent memory store: embed conversation turns, retrieve semantically"] },
  ...
  },
  {
    ...
    tracks: [
      ...
      { name: "Agent Memory Stack", items: [
        "Memory tiers: Hot (Redis) → Warm (PostgreSQL) → Semantic (pgvector) — when to use each",
        "Redis — RedisChatMessageHistory: store per-session message history with TTL auto-expiry",
        "Redis — LangGraph RedisSaver: fast ephemeral checkpointer for dev and session-scoped state",
        "Redis — HASH for agent state: store structured key-value agent context per thread_id",
        "Redis — LIST for message buffer: LPUSH / LRANGE / LTRIM for sliding window conversation",
        "Redis — ZSET for scored memory: store memories with recency score, retrieve top-N recent",
        "Redis — TTL strategy: short TTL for session (1hr), longer for user context (7 days)",
        "PostgreSQL — PostgresChatMessageHistory: persist full conversation history per session",
        "PostgreSQL — LangGraph PostgresSaver: production checkpointer with thread_id + checkpoint_id",
        "PostgreSQL — agent memory schema: agent_sessions, agent_messages, agent_checkpoints tables",
        "PostgreSQL — memory query patterns: fetch last N messages, fetch by date range, pagination",
        "PostgreSQL — JSONB for flexible memory: store tool results, agent metadata, structured facts",
        "pgvector — semantic memory store: embed each conversation turn, store with user_id + timestamp",
        "pgvector — episodic retrieval: search past interactions by semantic similarity to current query",
        "pgvector — fact memory: store extracted facts as embeddings, retrieve relevant ones at runtime",
        "Chroma — local vector memory: ChromaDB persistent store for dev, namespace per user/session",
        "Memory summarization: LLM summarizes old messages → store summary in PostgreSQL, clear buffer",
        "Memory injection pattern: at graph start, fetch Redis context + PostgreSQL history + top-k vector memories → build enriched system prompt",
        "mem0: AI memory layer — auto-extracts and manages memories from conversations",
        "Zep: purpose-built LLM memory store with temporal knowledge graph and fact extraction",
      ]},
    ...
    hands_on: [
      ...
      { title: "Tiered Memory Agent", desc: "Build the full 3-tier memory system: Redis (RedisChatMessageHistory + HASH for session state), PostgreSQL (PostgresChatMessageHistory + checkpointer), pgvector (embed conversation turns, retrieve top-3 semantically relevant past interactions). Wire all three into a single memory injection node that enriches the system prompt before each LLM call.", tags: ["Redis", "PostgreSQL", "pgvector", "LangGraph", "mem0"] },
  ...
  },
  {
    ...
    tracks: [
      ...
      { name: "Multi-Agent Memory", items: [
        "Shared Redis namespace: all agents read/write to common HASH key — cross-agent working memory",
        "Agent-isolated PostgreSQL stores: each agent type has its own message history table + namespace",
        "Shared pgvector collection: all agents upsert to common vector store, query with agent_type filter",
        "Global fact store: agents write extracted facts to pgvector, any agent retrieves relevant facts",
        "Supervisor memory: supervisor maintains PostgreSQL log of all subtask results + final synthesis",
        "Memory handoff: when agent A hands off to agent B, serialize A's relevant state into handoff payload",
        "Conflict resolution: last-writer-wins for Redis keys; append-only for PostgreSQL; deduplicate vectors",
        "Memory scoping: session-level (Redis TTL), user-level (PostgreSQL), global (pgvector facts)",
        "Cross-run continuity: retrieve past runs by user_id from PostgreSQL, inject as context into new run",
        "Memory pruning: scheduled job deletes expired Redis keys, archives old PostgreSQL rows, removes stale vectors",
      ]},
...
export default function Roadmap() {
  ...
  return (
    <div style={{ fontFamily: "'Outfit', sans-serif", background: p.tint, minHeight: "100vh", color: "#1a1a1a", transition: "background 0.45s ease" }}>
      <style>{`
      ...
      {/* TOP BAR */}
      <div style={{ background: "#1a1a1a", padding: "11px 48px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
      ...
      {/* HERO */}
      <div style={{ padding: "56px 48px 44px", maxWidth: "1280px", margin: "0 auto" }}>
      ...
      {/* DIVIDER */}
      <div style={{ height: "1px", background: p.midtint, maxWidth: "1280px", margin: "0 auto", transition: "background 0.4s" }} />

      {/* MAIN CONTENT */}
      <div style={{ maxWidth: "1280px", margin: "0 auto", padding: "0 48px 72px" }}>
